...
public class ShellUtils {
    ...
    public ShellUtils run(String command, final int maxTime) {
        spm("run command:" + command + ",maxtime:" + maxTime);
        if (command == null || command.length() == 0) {
            return this;
        }

        try {
            process = getRuntime().exec("su");//申请root权限
        } catch (Exception e) {
            spm("e:" + e.toString());
            return this;
        }
        ...
        try {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            spm( "exitValue Stream over" + ret);
                        } catch (IllegalThreadStateException e) {
                            spm( "IllegalThreadStateException:" + e.toString());
                            spm( "take maxTime,forced to destroy process");
                            process.destroy();
                        }
                    }
                }).start();
            ...
            final Thread t1 = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        spm( "read InputStream exception:" + e.toString());
                    } finally {
                        try {
                            successResult.close();
                            spm( "read InputStream over");
                        } catch (Exception e) {
                            spm( "close InputStream exception:" + e.toString());
                        }
                    }
                }
            });
            ...
            final Thread t2 = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        spm( "read ErrorStream exception:" + e.toString());
                    } finally {
                        try {
                            errorResult.close();
                            spm( "read ErrorStream over");
                        } catch (Exception e) {
                            spm( "read ErrorStream exception:" + e.toString());
                        }
                    }
            ...
            if (bSynchronous) {
                spm( "run is go to end");
                t3.join();
                spm( "run is end");
            }
        } catch (Exception e) {
            spm( "run command process exception:" + e.toString());
        }
        return this;
    }
